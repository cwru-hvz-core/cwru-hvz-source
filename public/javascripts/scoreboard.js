...
$(document).ready(function() {
  ...
  if (show_game_id == thisgame.game_id) {
    showgame = thisgame;
  } else {
    showgame = new Game({id: show_game_id, logged_in_user_id: show_game_user});
  }
...
///////////////////////////////////////////////////////////
  ...
  var options = {
    chart: {
      height: 140, defaultSeriesType: 'line', renderTo: 'graph', margin: [10, 0, 10, 30]
  ...
  function computePlayers() {
    ...
    if (!("info" in showgame.retrieved_data) ||
                            !("players" in showgame.retrieved_data)) {
      return;
    }
    ...
    $("td#total_count").html(showgame.players.length);
...
function populate_player_list() {
  ...
  if (showgame.logged_in_user_id != null) {
    for (var i in showgame.players_sorted) {
      if (showgame.players_sorted[i].id == showgame.logged_in_user_id) {
        $("div#logged_in_player").append(
          showgame.players_sorted[i].get_scoreboard_html(showgame.retrieved_data["info"]["now"])
        );
      }
    }
  }
  ...
  showgame.append_on_load_info("squads", function() {
    ...
    for (var i in showgame.squads_sorted) {
      ...
      $("div#squad_list").append(
        showgame.squads_sorted[i].get_scoreboard_html(showgame.retrieved_data["info"]["now"])
      );
    }
  })
} 
 ...
 *   Note: populate_zombie_tag_tree assumes that showgame.retrieved_data["tags"] exists.
...
function tag_tree_recursive(tags_by_tagger_id, tagger_id) {
  ...
  // and stop recursing.
  var p = showgame.players_by_id(tagger_id);
